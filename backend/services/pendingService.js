...
class PendingService{
    static async insertQuestion(difficulty, title, answer, description, hint, categoryList){
        try{
            const insertQuery = {
                text: 'INSERT INTO sql_tutoring."Pending" (title, difficulty, hint, answer, description, category) VALUES ($1, $2, $3, $4, $5, $6)',
                values: [title, difficulty, hint, answer, description, categoryList],
            };
    ...
    static async transferQuestion(questionId){
        ...
        try{
            await QuestionService.insertQuestion(res.title, res.difficulty, res.answer, res.description, res.hint, res.category)
